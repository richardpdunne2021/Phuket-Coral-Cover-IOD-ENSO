...
setwd ("E:/Phuket Databases/Photo Transects/Analysis")
...
# capture from Excel file using copy and importing the clipboard to R: the latest version of this input file for the original data series is "Coral Cover PhotoTransects 9,11,14,15.xlsx" and we will use the sheet"Time Series" up to the final complete data line for Nov 2020. Include the column names header.
Coral_cover <- read.table(file = "clipboard", sep = "\t", header=TRUE)
...
gX6 <- gX5 + geom_line(aes(y=(a+((Coral_cover$RTN_corrected_SLA-mean(Coral_cover$RTN_corrected_SLA))/sd(Coral_cover$RTN_corrected_SLA))*scalingfactor)), colour="magenta2", size=0.6) +
  ylim(ylim.prim) +
  scale_y_continuous(name="Coral Cover %",sec.axis=sec_axis(~(.- a)/scalingfactor*sd(Coral_cover$RTN_corrected_SLA)+mean(Coral_cover$RTN_corrected_SLA), name = "Sea Level (mm)"),) +
...
gX7 <- gX5 + geom_line(data=spline.df, aes(x=x,y=y), colour="black", size=1) +
...
gX8 <- gX7 + geom_line(aes(y=Coral_cover$DHW), colour="red", size=0.5) +
...
# SOME EXPERIMENTING HOW TO PLOT GRAPHS TOGETHER
...
# NOTE IN THIS SECTION COMPUTATIONS WERE DONE FOR FULL TIME SERIES ABOVE NO NEED TO REPEAT HERE - code kept for completeness
# add line for sea level and a second y axis to visualise this
# convert the sea level anomaly back to the original values and make this an extra column to avoid using negative values on this combined graph
# the mean we had used in "Monthly sea level at Tapao Noi 1977-2020.xlsx" was 2247
#Coral_cover$RTN_corrected_SLA <- Coral_cover$RTN_final_datums_corrected + 2247
...
# USING GAM TO OBTAIN A SPLINE THROUGH THE TRANSECT DATA WITH CONFIDENCE INTERVALS
...
# Our Data
require(mgcv)
fit1 <- gam(Mean ~ s(Decimal_date), data=Coral_cover)
plot (fit1)
with(Coral_cover, points(Decimal_date, Mean))
